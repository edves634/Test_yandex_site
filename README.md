...
- Кросс-браузерность (Chrome, Firefox, Яндекс.Браузер, WebKit)
...
| **Pixel Perfect Pro** | расширение для Chrome | Наложение PNG макета Figma поверх страницы, прозрачность 50% (проверка pixel perfect) |
| Firefox | 125.0.3 | Адаптивность, клавиатурная навигация |
| Яндекс.Браузер | 24.4.1 | Кросс-браузерность |
| WebKit | Playwright 1.40.0, эмуляция iPhone 12, iOS 15.5 | iOS-подобное окружение (реальный Safari не использовался) |
...
| WebKit | Playwright 1.40.0, эмуляция iPhone 12, iOS 15.5 | Несоответствие блока «Этапы преображения Васюков» (текст выходит за границы, не центрируется) на 375px | ❌ Критично |
...
- **❌ Неверный перенос в блоке «Этапы преображения Васюков»** – подзаголовок «Будущие источники обогащения васюкинцев» прилипает к слову «Васюков» (отсутствует перевод строки). Причина: у `.timeline-section__description` не задан `display: block`, элемент остаётся строчным/inline-flex на десктопе и не переносится на новую строку на мобильной ширине.
- **⚠️ Несоответствие шрифтов** – в макете для цифр (этапы) указана `line-height: 24px` (120% от 20px), в CSS значение не задано (браузерное ~22-24px). В бегущей строке Figma задаёт `line-height: 110%` (17.6px), в CSS используется `16.7px`. Отсутствует `font-feature-settings: 'pnum' on, 'lnum' on` для текстов с цифрами (в CSS используется `font-variant-numeric`, что даёт иной рендеринг).
- **❌ Отсутствие многослойного декоративного фона** в блоке «Этапы преображения Васюков» (вместо сложной композиции из 10+ слоёв – одно упрощённое изображение `timeline-card__img-mob.png`).
...
📸 **Скриншоты проблем** находятся в папке `screenshots` (файлы: `768px_timeline_broken.png`, `1920px_header_only.png`, `safari_webkit_375px_timeline.png`, `375.png` – наложение для переноса, **`375-2.png`** – наложение для декоративного фона блока «Этапы преображения Васюков»).

---

## 5. Соответствие вёрстки макету Figma (Pixel Perfect)

**Метод проверки:** расширение **Pixel Perfect Pro** для Chrome (наложение PNG макета поверх страницы, прозрачность 50%).
...
| **Этапы преображения Васюков (подзаголовок)** | **«Будущие источники обогащения васюкинцев» прилипает к слову «Васюков» – нет переноса строки. Причина: у `.timeline-section__description` отсутствует `display: block`. В Figma подзаголовок всегда начинается с новой строки.** | 🔴 Высокая |
| **Шрифты (типографика)** | **1. Цифры в этапах (`.num`): в макете `line-height: 24px` (120% от 20px), в CSS значение не задано → разница в вертикальном ритме. 2. Бегущая строка (`.marquee__text li`): Figma `line-height: 110%` (17.6px), CSS `16.7px`. 3. Отсутствует `font-feature-settings: 'pnum' on, 'lnum' on` для текстов с цифрами (вместо него `font-variant-numeric` даёт иную гарнитуру.** | 🟡 Средняя |
| **Декоративный фон блока «Этапы преображения Васюков»** | **В Figma присутствует сложная композиция из 10+ слоёв (штамп Cooper Union, купюра, жёлтая плашка, маски, изображение шахматиста, шахматная доска, текстура). В вёрстке — одно примитивное изображение `timeline-card__img-mob.png`, которое полностью заменяет все эти детали, нарушая замысел дизайна.** | 🔴 Высокая |

📸 **Скриншоты наложения Pixel Perfect для блока «Этапы преображения Васюков»:**  
- `375.png` – демонстрирует проблему переноса (прилипание подзаголовка).  
- `375-2.png` – демонстрирует отсутствие многослойного декоративного фона (видна только одна картинка вместо сложной композиции).
...
| **У `.timeline-section__description` нет `display: block`** | ❌ | Добавить `display: block; width: 100%;` (исправит перенос) |
| **Не задан `line-height` для `.num`** | ❌ | Добавить `line-height: 2.4rem` или `24px` |
| **Нет `font-feature-settings` для цифр** | ❌ | Заменить `font-variant-numeric` на `font-feature-settings: 'pnum' on, 'lnum' on` |
...
| ID | Описание (кратко) | Блок | Приоритет | Статус |
|----|-------------------|------|-----------|--------|
| BUG-06 | Конфликт slider.js и media.js, ломает DOM при ресайзе | JS | 🔴 Critical | Open |
| BUG-01 | Отсутствуют многослойные декоративные фоны | Весь сайт | 🔴 High | Open |
| BUG-02 | В бегущих строках только 1–2 фразы вместо 3 | Hero + футер | 🔴 High | Open |
| BUG-03 | Лишний блок .mob в секции поддержки турнира | Десктоп | 🔴 High | Open |
| BUG-04 | Высота шапки 97px (должна быть 64px на десктопе) | Header | 🔴 High | Open |
| BUG-05 | Одинаковые фото, нет круглой подложки, неверный титул Бендера | Карусель участников | 🔴 High | Open |
| BUG-07 | Вёрстка для 320px отсутствует (обрезка текста, карточек) | Адаптивность | 🔴 High | Open |
| **BUG-11** | **Подзаголовок «Будущие источники…» прилипает к «Васюков» – нет `display: block`** | **Этапы (375px)** | **🔴 High** | **Open** |
| **BUG-13** | **В блоке «Этапы преображения Васюков» (мобильная версия) отсутствует многослойный декоративный фон согласно Figma (штамп, купюра, жёлтая плашка, шахматист, доска и др.). Вместо этого одно несоответствующее изображение.** | **Этапы (375px)** | **🔴 High** | **Open** |
| BUG-08 | На 768px сетка этапов ломается, текст обрезается справа | Этапы | 🟡 Medium | Open |
| BUG-09 | Отсутствует `<main>`, кнопки без aria-label | Accessibility | 🟡 Medium | Open |
| BUG-10 | Авто-пролистывание карусели на мобильных не отключается | Карусель участников | 🟡 Medium | Open |
| **BUG-12** | **Несоответствие line-height в цифрах этапов и бегущей строке; отсутствие font-feature-settings** | **Типографика** | **🟡 Medium** | **Open** |

---

## 8. Рекомендации по исправлению (по приоритетам)

### 🔴 Critical (блокеры релиза – исправить до любого показа)

1. **Удалить media.js** – перенести всю адаптацию на CSS Grid/Flex

### 🔴 High (серьёзные – исправить в первую очередь)
...
7. **Исправить перенос в блоке «Этапы преображения Васюков»** – добавить в CSS:
   ```css
   ...
   ```
8. **Восстановить сложный декоративный фон в блоке «Этапы преображения Васюков»** согласно Figma:  
   - убрать одиночную картинку `.slider__card_img`  
   - создать контейнер с относительным позиционированием  
   - абсолютно расположить все элементы: штамп, купюру, жёлтую плашку, фотографию шахматиста, шахматную доску, текстуру  
   - применить `mix-blend-mode`, `opacity`, `transform` (повороты, матрицы) как в макете  
   - обеспечить корректное наложение поверх карточек этапов (с учётом z-index)
...
6. **Привести типографику к макету**:
   - Для `.num`: `line-height: 2.4rem` или `24px`
   - Для `.marquee__text li`: `line-height: 1.1` (17.6px)
   - Заменить `font-variant-numeric` на `font-feature-settings: 'pnum' on, 'lnum' on` для всех текстов с цифрами
...
- **Нарушение переноса строки в блоке «Этапы преображения Васюков»** (подзаголовок прилипает к заголовку)
- **Неточности в типографике** (line-height цифр, бегущей строки, отсутствие font-feature-settings)
- **В блоке «Этапы преображения Васюков» на мобилке вместо многослойного дизайна используется упрощённое изображение, не соответствующее Figma**

**Итог:** сайт требует полной ревизии вёрстки с упором на pixel perfect по макету Figma, переработки JS-логики (удаление media.js) и добавления резиновой/адаптивной сетки. Необходимо также исправить обнаруженные типографические расхождения, обеспечить корректный перенос текста в подзаголовке на мобильной версии и **восстановить сложный декоративный фон в блоке «Этапы преображения Васюков»**.
...
**Ответ:**  
Я бы построила процесс в соответствии с **Agile / Scrum**, интегрируя тестирование на всех этапах:
